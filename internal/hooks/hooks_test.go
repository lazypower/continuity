...
func TestHasSignal(t *testing.T) {
	tests := []struct {
		...
		{"we decided to use Go", false},
		{"this pattern works well for concurrent access", false},
		{"the trick is to use buffered channels", false},
		{"the bug was in the connection pool", false},
