...
| **L2** | Unlimited | Full original content. | On-demand deep retrieval only |

*Revised in v0.11.0 (issue #72): L2 previously read "Raw observations, full transcripts." Raw observations are no longer a permanent L2 surface — see the revision note in §4.3.*
...
> **Revised in v0.11.0 (issue #72): `mem://sessions/abc123/observations` is withdrawn.**
>
> This RFC promised permanent, browsable per-session raw observations as an L2
> surface. That promise turned out to be unaffordable, and we found out from a
> user rather than from ourselves: the `observations` table had no retention path
> and reached 331,966 rows / 1,036 MB in ~4.5 months of single-user use, against
> 3 MB of actual memories. At that size `/api/search` exceeded the CLI's client
> timeout and every command reported the server as down while the daemon was
> healthy — the growth was invisible until it surfaced as an unrelated-looking
> failure.
>
> Keeping the promise would have meant retaining every tool-use record forever, at
> roughly 230 MB per month per active user, to serve a surface nothing had ever
> called. We chose bounding over browsing: observations are now retained only while
> their session is in flight, and `GetObservations` / `GetRecentObservations`
> remain in the store layer but address only live sessions.
>
> What is genuinely lost: you cannot browse the raw tool-by-tool history of a
> session that ended more than the retention window ago. What is preserved: the
> memories extracted from that session, its summary, and its L1 — extraction reads
> the transcript file, not this table, so nothing that reaches the memory tree
> depended on the observations we now reclaim.
>
> If per-session raw history is wanted later it needs its own design with a cost
> model attached — a bounded ring buffer, an opt-in archive, or export-on-complete.
> It should not come back as an unbounded table.
