...
| `session_id` | string | no | the server's session | Provenance attribution. Defaults to the Claude Code session that started the MCP server (`CLAUDE_CODE_SESSION_ID`), which gives the memory project affinity. `show` and `search` attribute their journal events the same way. |
