...
| `--session <id>` | *(empty)* | Attribute the write to a session, for provenance. Run from inside a Claude Code session, the write is attributed to that session (`CLAUDE_CODE_SESSION_ID`) and this flag is ignored. The session gives the memory project affinity. |
